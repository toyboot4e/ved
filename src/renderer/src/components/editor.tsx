...
// --- Cursor position mapping between plain and rich editors ---

/** Compute the plain-text character count that a single rich child contributes. */
const richChildPlainLength = (child: Descendant): number => {
...
const richOffsetToPlain = (richChildren: Descendant[], childIndex: number, offset: number): number => {
  let plainOffset = 0;
...
const rubyBodyLength = (ruby: { children: Descendant[] }): number =>
...
const rubyLocalToBodyOffset = (bodyLen: number, local: number): number => {
  if (local <= 0) return 0;
