...
| 2 | **The fcitx window is guarded below the composing caret (Linux/X11)** | fcitx5's own window placement | fcitx places its input window per key event, from the caret rect it holds AT THAT MOMENT; Chromium's fresh rect (after the preedit change lays out) round-trips asynchronously, so a key RELEASE within a few ms of the press — a mod-tap keyboard resolves the tap ON release — is processed with the STALE pre-compose rect and the window opens ON the first preedit cell, covering it. Nothing repositions it: fcitx ignores rect-only updates while mapped (a held mid-composition `selection.collapse` never moves it — mozc-verified), and a single-character composition gets no further engine update. Unfixable in the renderer, so MAIN corrects post-hoc: the editor reports the live composing caret rect per update (`onImeCaretRect` — an optional, platform-neutral prop; null at compositionend), the shell streams it over `IpcChannel.ImeCaretRect`, and the guard polls the fcitx window via xdotool while composing, moving any window that sits ABOVE the caret's bottom down below it. Moving fcitx's override-redirect window is tolerated: no snap-back, and mozc composes/converts/commits through it. Inert off X11 / without xdotool; touches nothing when fcitx places correctly | desktop `main/ime-window-guard.ts`, `ime-caret-pin.ts` |
