...
| 2,3 | **Every scroll offset is held while composing** | Blink reveal-scrolls the selection on each composition update | a band-crossing preedit's DOM caret transiently leaves the viewport and the native reveal scrolls whatever moves — the paged scroller vertically, a shell ancestor or the document horizontally when a band overflows the window — so the whole page wobbled a column per keystroke ("Keeping the caret in view"). Ancestor-chain snapshot at compositionstart, restored on any scroll, released with one reveal at compositionend | `ime-scroll-hold.ts` |
...
While COMPOSING nothing may scroll — ours is composition-gated, and
`ime-scroll-hold.ts` holds Blink's native selection reveal too: Blink
reveal-scrolls on every composition update, and a band-crossing preedit's
DOM caret transiently sits outside the viewport, so the reveal yanked the
paged scroller vertically and — when a band is wider than the window — a
shell ancestor (or the document) horizontally, wobbling every client rect
a whole column per keystroke (the `mozc/candidate-window-pos` border
stray). The hold snapshots every ancestor's scroll offsets at
compositionstart and restores them on any scroll (inside the scroll steps,
before the frame paints); compositionend releases with one normal reveal.
No DOM, selection, or focus is touched — IME-safe.
