...
const capture = async (
  ...
  if (toStart) {
    ...
    await page.evaluate(() => {
      // The editor scroller is the element with overflow-x scroll; in
      // vertical-rl the document start is the rightmost scroll position.
      const cand = Array.from(document.querySelectorAll('div')).find((d) => getComputedStyle(d).overflowX === 'scroll');
      if (cand) cand.scrollLeft = cand.scrollWidth;
    });
    await page.waitForTimeout(200);
  }
...
await capture('Vertical', `${root}cap-vertical.png`);
await capture('Vertical Columns', `${root}cap-columns.png`);
