...
- **absolutely positioned** at the line's **inline-START** edge
  (`inset-inline-start`/`inset-block-start: 0`) — which is the LEFT in
  horizontal and the **TOP of each column** in vertical-rl, via logical props;
- forced **`writing-mode: horizontal-tb`** so the digits never stack;
