 ...
 *  original anchor. A geometric hit-test can land the caret on hidden markup or
 *  a collapsed ruby's read-only reading — neither hosts a DOM caret, so
 *  committing it resyncs the selection to offset 0 (the "left-key jump"). Snap
 *  such a landing onto the nearest renderable base glyph. Plain text is
 *  unaffected. */
...
/** Is modify's landing a REAL line step, or a stray slide to the paragraph
 *  edge? At a paragraph's first/last visual line `modify('line')` has no line
 *  to step to, so it slides to the line start/end — landing EXACTLY at the
 ...
 *  (`isRealLineStep`) and return its PM pos; null rejects a stray slide to the
 *  paragraph edge so it falls through to column-step / sibling-cross / revert,
 ...
 *  (the live DOM caret rect, captured before `modify` ran) gives the caret's
 *  column reliably — including at the doc end, where the *model* rect
 *  `coordsAtPos(head)` instead reports the empty next column. At a ruby
 *  BOUNDARY (between two collapsed atom rubies, no text node) the DOM rect is
 *  degenerate (0×0); fall back to the model rect there.
 *
 ...
 *  RUN (`goalHeld`) the caret reached this seam by landing on a column start,
 *  so resolve the ambiguity with the AFTER-side model rect
 *  (`coordsAtPos(head, 1)`), which reports the column whose start is the seam.
 *  Apply it ONLY when the two affinities straddle a column boundary AND the
 *  after side lands on a REAL column — so the doc/paragraph END (where the
 *  after side is the empty next column) keeps `beforeRect` (the true last
 *  column) and does not over-step. */
...
const adjacentTargetCol = (
  ...
  if (targetP?.tagName !== 'P') return null; // document edge
