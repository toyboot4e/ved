...
// default Mod+,). The panel hosts the runtime configuration controls (view
// config, invisibles); the controls write their own stores, so this store is
// UI state only — nothing here persists (the Vim model: `init.ts` is the
// durable config). Non-modal:
